...
class ContentPlan:
    ...
    def _create_default(cls, topic: str) -> 'ContentPlan':
        sections = [
            Section(title="Introduction", description=f"Overview of {topic}", index=0),
            Section(title="Main Discussion", description=f"Detailed analysis of {topic}", index=1),
            Section(title="Conclusion", description="Summary and key takeaways", index=2)
        ]
        return cls(
            topic=topic,
...
class ContentCreationSystem:
    ...
    def generate_markdown_with_citations(
        ...
        """Generate markdown with improved citations"""
        try:
            markdown_content = [
                f"# {content_plan.topic}\n",
                content_plan.introduction or "",
                "\n---\n"
            ]

            # Add sections with in-text citations
            for section in content_plan.sections:
                if section.content:
            ...
            if content_plan.conclusion:
                ...
                markdown_content.append(content_plan.conclusion)

            # Add references section with detailed citations
            if self.citations:
                markdown_content.extend([
                    "\n## References\n",
